<div class="row">
    <div class="col-md-12">
        <a href='{{ route('board.educational_systems.create') }}' class='btn btn-primary mb-2' style="float: left;">  <i class="icon-plus3 me-2"></i>  
            @lang('educational_systems.add new educational system')
        </a>
    </div>
    <div class="col-md-12">
        <div class="card">
            <div class="card-header bg-primary text-white d-sm-flex align-items-sm-center ">
                <h5 class="mb-0"> @lang('educational_systems.show all educational systems') </h5>
            ...
            <div class='card-body' >
                <table  class='table  table-responsive table-striped table-xs text-center '>
                    <thead>
                        <tr>
                            <th> # </th>
                            <th> @lang('educational_systems.name in arabic') </th>
                            <th> @lang('educational_systems.name in english') </th>
                            <th> @lang('dashboard.added by') </th>
                            <th> @lang('educational_systems.status') </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td> {{ $i++ }} </td>
                            <td> {{ $system->getTranslation('name'  , 'ar') }} </td>
                            <td> {{ $system->getTranslation('name'  , 'en') }} </td>
                            <td> {{ $system->user?->name }} </td>
                            <td> 
                                @switch($system->is_active)
                                    @case(1)
                            ...
                            <td>
                                <a href='{{ route('board.educational_systems.edit' , $system ) }}' class='btn btn-sm btn-warning ' title="@lang('dashboard.edit')" >  <i class="icon-database-edit2 "></i>  </a>

                                <a wire:click="$dispatch('deleteConfirmation', '{{ $system->id }}')" class='btn btn-sm btn-danger  delete_item' title="@lang('dashboard.delete')" >  <i class="icon-trash "></i>  </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
