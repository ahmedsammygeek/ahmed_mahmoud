    ...
    <div class="col-md-12">
        ...
        <table  class='table  table-responsive table-striped table-xs text-center '>
            <thead>
                <tr>
                    <th> # </th>
                    <th> @lang('students.lesson title') </th>
                    <th> @lang('students.unit') </th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td> {{ $lesson->deleted_at }} - <span class='text-muted'> {{ $lesson->deleted_at?->diffForHumans() }} </span> </td>
                    <td> {{ $lesson->deletedBy?->name }}  </td>
                    <td>

                        <a wire:click="$dispatch('restoreConfirmation', '{{ $lesson->id }}')" class='btn btn-sm btn-primary  delete_item' title="@lang('dashboard.restore')" >  <i class="icon-transmission "></i>  </a>
                        <a wire:click="$dispatch('deleteConfirmation', '{{ $lesson->id }}')" class='btn btn-sm btn-danger  delete_item' title="@lang('dashboard.delete')" >  <i class="icon-folder-remove "></i>  </a>
                    </td>
                </tr>
                @endforeach
            </tbody>
