    ...
    <div class="col-md-12">
        ...
        <table  class='table  table-responsive table-striped table-xs text-center '>
            <thead>
                <tr>
                    <th> # </th>
                    <th> @lang('students.student name') </th>
            ...
            <tbody>
                ...
                <tr>
                    <td> {{ $i++ }} </td>
                    <td> {{ $students_course->student?->name }} </td>
                    <td> {{ $students_course->course?->title }} </td>
                    <td> {{ $students_course->deleted_at }} - <span class='text-muted'> {{ $students_course->deleted_at?->diffForHumans() }} </span> </td>
                    <td> {{ $students_course->deletedBy?->name }}  </td>
                    <td>

                        <a wire:click="$dispatch('restoreConfirmation', '{{ $students_course->id }}')" class='btn btn-sm btn-primary  delete_item' title="@lang('dashboard.restore')" >  <i class="icon-transmission "></i>  </a>
                        <a wire:click="$dispatch('deleteConfirmation', '{{ $students_course->id }}')" class='btn btn-sm btn-danger  delete_item' title="@lang('dashboard.delete')" >  <i class="icon-folder-remove "></i>  </a>
                    </td>
                </tr>
                @endforeach
            </tbody>
