<div class="row">
    <div class="col-md-12">
        <a href='{{ route('board.faculty_levels.create') }}' class='btn btn-primary mb-2' style="float: left;">  <i class="icon-plus3 me-2"></i>  
           إضافه جديد
        </a>
    </div>
    <div class="col-md-12">
        <div class="card">
            <div class="card-header bg-primary text-white d-sm-flex align-items-sm-center ">
                <h5 class="mb-0"> @lang('faculty_levels.show all faculties Levels') </h5>
            ...
            <div class='card-body' >
                <table  class='table  table-responsive table-striped table-xs text-center '>
                    <thead>
                        <tr>
                            <th> # </th>
                            <th> @lang('faculty_levels.name in arabic') </th>
                            <th> @lang('faculty_levels.name in english') </th>
                            <th> @lang('faculty_levels.faculty') </th>
                            <th> @lang('dashboard.added by') </th>
                            <th> @lang('faculty_levels.status') </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td> {{ $i++ }} </td>
                            <td> {{ $level->getTranslation('name'  , 'ar') }} </td>
                            <td> {{ $level->getTranslation('name'  , 'en') }} </td>
                            <td> {{ $level->faculty?->name }} </td>
                            <td> {{ $level->user?->name }} </td>
                            <td> 
                                @switch($level->is_active)
                                    @case(1)
                            ...
                            <td>
                                 <a href='{{ route('board.faculty_levels.show' , $level ) }}' class='btn btn-sm btn-primary ' title="@lang('dashboard.view')" >  <i class="icon-eye "></i>  </a>


                                <a href='{{ route('board.faculty_levels.edit' , $level ) }}' class='btn btn-sm btn-warning ' title="@lang('dashboard.edit')" >  <i class="icon-database-edit2 "></i>  </a>

                                <a wire:click="$dispatch('deleteConfirmation', '{{ $level->id }}')" class='btn btn-sm btn-danger  delete_item' title="@lang('dashboard.delete')" >  <i class="icon-trash "></i>  </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
