...
<span class="breadcrumb-item active"> إضافه صنف جديد </span>
@endsection
	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header bg-primary text-white">
				<h5 class="mb-0"> إضافه صنف جديد </h5>
			</div>

			<form class="" method="POST" action="{{ route('board.items.store') }}" enctype="multipart/form-data" >
				<div class="card-body">
					@csrf
					<div class="mb-4">
						...
						<div class="row mb-3">
							...
							<div class="col-lg-10">
								<input type="text" name="name_ar" value="{{ old('name_ar') }}" class="form-control @error('name_ar')  is-invalid @enderror" required placeholder="اسم الصنف">
								@error('name_ar')
								<p class='text-danger' > {{ $message }} </p>
								@enderror
							</div>
						...
						<div class="row mb-3">
							...
							<div class="col-lg-10">
								<input type="text" name="name_en" value="{{ old('name_en') }}" class="form-control @error('name_en')  is-invalid @enderror" required placeholder="اسم الصنف">
								@error('name_en')
								<p class='text-danger' > {{ $message }} </p>
								@enderror
							</div>
						...
						<div class="row mb-3">
							...
							<div class="col-lg-10">
								<input type="text" name="points" value="{{ old('points') }}" class="form-control @error('points')  is-invalid @enderror" required placeholder="عدد نقاط الصنف عند الاستبدال">
								@error('points')
								<p class='text-danger' > {{ $message }} </p>
								@enderror
							</div>
